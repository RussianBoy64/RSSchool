;(() => {
  ...
  const e = {
      en: {
        links: {
          main: { href: './', text: 'Main', className: 'nav__link' },
          quiz: { href: './quiz', text: 'Quiz', className: 'nav__link' },
          gallary: {
            href: '/gallery',
            text: 'Gallery',
            className: 'nav__link',
    ...
    t = new (class {
      ...
      startQuiz = () => {
        ;(this.score = 0), (this.currentQuestion = 0), this.shuffleQuestions()
      }
    })(),
    { lang: s, answers: a, currentQuestion: i, score: o } = t.loadQuiz()
  s && (t.lang = s),
    a && (t.answers = a.split('')),
    0 != +i && (t.currentQuestion = +i),
    0 != +o && (t.score = +o),
    console.log(t)
  const n = t
  async function r() {
  ...
  async function c(e, t, s = 'link') {
    const a = document.createElement('a')
    return a.classList.add(s), (a.href = e), (a.textContent = t), a
  }
  async function d(t) {
    const s = document.createElement('main'),
      a = await r()
    if ((s.classList.add('main'), '/russianboy64-JSFE2022Q3/birdsong/' === t)) {
      const e = await (async function () {
        const e = n.lang,
          ...
          a = await c(
            './quiz',
            'en' === e ? 'Start Quiz' : 'Начать викторину',
            'link'
          )
        return (
          t.classList.add('main__inner'),
          s.classList.add('main__title'),
          (s.innerHTML =
            'en' === e
          ...
          t.append(s),
          t.append(a),
          a.addEventListener('click', n.startQuiz),
          t
        )
      })()
      a.append(e)
      ...
      const t = await (async function () {
        const { lang: t, currentQuestion: s, score: a, answers: i } = n,
          o = document.createElement('div'),
          r = await (async function (t, s, a) {
            const i = document.createElement('section'),
              o = document.createElement('div'),
              n = document.createElement('span'),
              r = e[t].questions
            ;(n.textContent = 'en' === t ? 'Score: ' : 'Счет: '),
              (n.textContent += a),
              i.classList.add('quiz__questions'),
              o.classList.add('questions__wrapper'),
              n.classList.add('questions__score'),
              i.append(n),
              i.append(o)
            for (let e in r) {
              const t = document.createElement('span')
              ;(t.textContent = r[e]),
                t.classList.add('questions__question'),
                e == s && t.classList.add('active'),
                o.append(t)
            }
            return i
          })(t, s, a),
          c = await (async function (t, s, a) {
            const i = document.createElement('section'),
              o = document.createElement('div'),
              n = document.createElement('span'),
              r = await (async function (t, s, a) {
                const i = document.createElement('div'),
                  ...
                  m = new Audio(e[t].birds[s][a[s] - 1].audio)
                return (
                  i.classList.add('player'),
                  o.classList.add('player__play-btn'),
                  n.classList.add('player__volume'),
                  r.classList.add('volume__btn'),
                  c.classList.add('volume__levelbar'),
                  d.classList.add('volume__level'),
                  l.classList.add('player__song'),
                  p.classList.add('song__progressbar'),
                  u.classList.add('song__progress'),
                  h.classList.add('song__info'),
                  (o.innerHTML = '<i class="fa-solid fa-play"></i>'),
                  (r.innerHTML = '<i class="fa-solid fa-volume"></i>'),
            ...
            return (
              i.classList.add('quiz__current-question'),
              o.classList.add('current-question__img'),
              n.classList.add('current-question__bird'),
              i.append(o),
              i.append(n),
              i.append(r),
              i
        ...
        return o.classList.add('main__inner'), o.append(r), o.append(c), o
      })()
      a.append(t)
    }
    return s.append(a), s
  ...
  async function p(e, t) {
    ...
    return (
      (s.htmlFor = e), (s.textContent = t), s.classList.add('lang__label'), s
    )
  }
  async function u(e) {
    e.preventDefault()
    ...
    const a = document.querySelector('.header'),
      ...
      r = await d(s)
    a.replaceWith(i), o.replaceWith(r)
  }
  async function h(t) {
    const s = document.createElement('header'),
      ...
      i = await (async function () {
        ...
        return (
          ...
          s.classList.add('logo__text'),
          (e.href = './'),
          (t.src = './assets/logo.png'),
          (t.alt = 'application logo'),
          (s.textContent = 'Birdsong'),
          e.append(t),
          e.append(s),
          e
        )
      ...
      o = await (async function (t) {
        const s = document.createElement('nav'),
          ...
          i = e[t].links
        s.classList.add('nav'), a.classList.add('nav__list')
        for (let e in i) {
          const { href: t, text: s, className: o } = i[e],
            r = await c(t, s, o),
            d = document.createElement('li')
          d.classList.add('list__item'),
            d.append(r),
            a.append(d),
            'quiz' === e && r.addEventListener('click', n.startQuiz)
        }
        return s.append(a), s
      })(t),
      d = await (async function (e) {
        const t = document.createElement('form'),
          s = await l('langControl', 'en', 'en', 'en' === e),
          a = await l('langControl', 'ru', 'ru', 'ru' === e),
          i = await p('en', 'en'),
          o = await p('ru', 'ru'),
          n = document.createElement('span')
        return (
          t.classList.add('lang__wrapper'),
          (n.textContent = '/'),
          t.append(s),
          t.append(i),
          t.append(n),
  ...
  !(async function () {
    const e = await h(n.lang),
      ...
      s = await (async function () {
        ...
        s.classList.add('author-info')
        const a = document.createElement('div'),
          i = document.createElement('a'),
          o = document.createElement('img')
        a.classList.add('author-info__item'),
          (i.href = 'https://rs.school/js/'),
          (o.src = 'https://rs.school/images/rs_school_js.svg'),
          (o.alt = 'RS School'),
          i.append(o),
          a.append(i)
        const n = document.createElement('div'),
          c = document.createElement('span')
        n.classList.add('author-info__item'),
          (c.textContent = '2022'),
          n.append(c)
        ...
        return (
          d.classList.add('author-info__item'),
          l.classList.add('github-link'),
          (p.href = 'https://github.com/RussianBoy64'),
          (p.textContent = 'RussianBoy64'),
          u.classList.add('fa-brands'),
          u.classList.add('fa-github'),
          u.classList.add('github-icon'),
          p.prepend(u),
          l.append(p),
          d.append(l),
          s.append(a),
    ...
    if (
      ...
      m.append(t),
      m.append(s),
      '/russianboy64-JSFE2022Q3/birdsong/' === g)
    ) {
      const e = await (async function () {
        const e = document.createElement('div'),
        ...
        return (
          ...
          (t.autoplay = !0),
          (t.muted = !0),
          (s.src = './assets/birdBackground.mp4'),
          (s.type = 'video/mp4'),
          e.classList.add('video__wrapper'),
          t.classList.add('video__background'),
          t.append(s),
          e.append(t),
          e
        )
    ...
    window.addEventListener('beforeunload', n.saveQuiz)
  })()
})()
